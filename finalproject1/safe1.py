...
#             self._buttons.append(button)
        ...
        message = Label(window, textvariable = self._message )
        message.grid(row = self._game.get_num()+1, column = 0, columnspan = 2, sticky = W)
        highscore = Label(window, text="Highest Score: ")
        highscore.grid(row = self._game.get_num()+1, column = 2, sticky = W)
        currentscore = Label(window, text="Current Score: ")
        currentscore.grid(row = self._game.get_num()+2, column = 2, sticky = W)
        self._current_label = IntVar()
        self._current_label.set(self._game.get_current_score())
        currentscore_label = Label(window, textvariable = self._current_label)
        currentscore_label.grid(row = self._game.get_num()+2, column = 3, sticky = W)
        add_button = Button(window, text = 'Play Game', command = self.start)
        add_button.grid(row = self._game.get_num()+3, column = 0, sticky = NSEW)
        add_button = Button(window, text = 'How to Play', command = showinstructions)
        add_button.grid(row = self._game.get_num()+3, column = 1, sticky = NSEW)
        add_button = Button(window, text = 'Quit Game', command = root.quit)
        add_button.grid(row = self._game.get_num()+3, column = 2, sticky = NSEW)
        #creating menubar
        menubar = Menu(root)
        #create a file menu tab
        filemenu = Menu(menubar, tearoff = 0)
        filemenu.add_command(label="New Game", command = self.draw_starting_board)
...
#             
# 
#         elif self._game.get_current_score() == 14:
#             self._game.add_num()
#             self._game.reset_num_level()
#             self.draw_button()
